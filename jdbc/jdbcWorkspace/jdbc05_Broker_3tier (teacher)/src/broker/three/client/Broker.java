...
/*
 * 실행 순서
 * ProtocolHandler -> StreamingData(초록색 지나가는 글씨) -> Broker(실시간 전광판)
 */
...
public class Broker implements ActionListener,ItemListener, Runnable{
	...
	public void createGUI() {	
		...
		/////////////////////// Ticker Tape 붙이기 //////////////////
		// Ticker Tape 전광판 효과 클래스
		TickerTape tt = new TickerTape("127.0.0.1", 700);
		tt.setSize(700, 300);
		new Thread(tt).start();
		
		frame.add(tt,"North"); //N을 대문자로 해줘야 위에 붙음.
	...
	*/
	...
	@Override
	public void run() {
		//무한루핑 돌면서... 10초 간격으로 디비의 비즈니스 로직을 호출한다...
		while(true) {
			try {
				showStockList(db.getAllStocks(), stockList);
				// 긁어온 주식 정보를 리스트에 붙임.
				System.out.println("$$$$$$ 실시간 주식정보....$$$$$$$");
				Thread.sleep(10000); //1초가 1000
			}catch(Exception e) {
